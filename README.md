...
```
composer require themazim/cassandra
```
...
```php
themazim\Cassandra\CassandraServiceProvider::class,
```
...
```php
$app->register('themazim\Cassandra\CassandraServiceProvider');
...
```php
use themazim\Cassandra\Eloquent\Model as Eloquent;
...
```php
'Moloquent'       => 'themazim\Cassandra\Eloquent\Model',
```
...
```php
'themazim\Cassandra\Auth\PasswordResetServiceProvider',
```
...
```php
use themazim\Cassandra\Eloquent\SoftDeletes;
...
*There is also support for upsert operations, check https://github.com/themazim/laravel-cassandra#cassandra-specific-operations*
...
If you're using a hybrid Cassandra and SQL setup, you're in luck! The model will automatically return a Cassandra- or SQL-relation based on the type of the related model. Of course, if you want this functionality to work both ways, your SQL-models will need use the `themazim\Cassandra\Eloquent\HybridRelations` trait. Note that this functionality only works for hasOne, hasMany and belongsTo relations.

Example SQL-based User model:

```php
use themazim\Cassandra\Eloquent\HybridRelations;
